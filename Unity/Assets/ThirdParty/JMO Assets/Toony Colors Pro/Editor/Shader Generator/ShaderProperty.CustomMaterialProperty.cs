	...
	namespace ShaderGenerator
	{
		public partial class ShaderProperty
		{
			...
			public class CustomMaterialProperty : IMaterialPropertyName
			{
				...
				internal void WillBeRemoved(Config config)
				{
					implementation.WillBeRemoved();

					foreach (var shaderProperty in config.AllShaderProperties)
					{
						shaderProperty.OnCustomTextureRemoved(this);
						foreach (var clonedShaderProperty in shaderProperty.clonedShaderProperties.Values)
						{
							clonedShaderProperty.OnCustomTextureRemoved(this);
						}
					}
				}
