	...
	namespace Demo
	{
		public class TCP2_Demo_Interactive : MonoBehaviour
		{
			...
			void Awake()
			{
				...
				cameraResetPos = cam.transform.position;
				cameraResetQuat = cam.transform.rotation;
			...
			{
				var screenPos = cam.WorldToScreenPoint(currentContent.pivot.position);
				var endPos = cam.WorldToScreenPoint(currentContent.textBox.position);
			...
			IEnumerator CR_ResetCamPos()
			{
				...
				while (time > 0)
				{
					...
					cam.transform.position = Vector3.Lerp(startPos, endPos, delta);
					cam.transform.rotation = Quaternion.Slerp(startQuat, endQuat, delta);
				}
			}
			...
			IEnumerator CR_MoveToContent(TCP2_Demo_Interactive_Content content)
			{
				...
				cam.transform.position = currentContent.transform.position;
				cam.transform.rotation = currentContent.transform.rotation;
