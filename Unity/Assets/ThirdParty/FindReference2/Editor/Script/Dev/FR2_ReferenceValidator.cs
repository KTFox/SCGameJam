...
namespace vietlabs.fr2
{
    ...
    /// </summary>
    internal class FR2_ReferenceValidator
    {
        ...
        public void ValidateAllReferences(bool exportToFile = false)
        {
            ...
            var criticalAssets = FR2_Cache._inst.AssetList
            ...
            FR2_LOG.Log($"[FR2_VALIDATION] Analyzing {result.totalAssets} critical assets...");
            ...
            foreach (var asset in criticalAssets)
            {
                ...
                if (progressPercent >= lastProgressReport + 10)
                {
                    ...
                    FR2_LOG.Log($"[FR2_VALIDATION] Progress: {progressPercent}% ({processedCount}/{result.totalAssets})");
                }
                ...
                if (difference.missingInFR2.Count > 0 || difference.extraInFR2.Count > 0)
                {
                    ...
                    string assetExt = FR2_StringCache.GetCachedExtension(asset.assetPath);
                    ...
                    foreach (var missing in difference.missingInFR2)
                    {
                        ...
                        string depExt = FR2_StringCache.GetCachedExtension(depPath);
        ...
        private AssetDifference CompareAssetReferences(FR2_Asset asset)
        {
            ...
            try
            {
                ...
                foreach (string depPath in unityDeps)
                {
                    ...
                    string depGuid = FR2_Cache.AssetPathToGUID(depPath);
                ...
                foreach (string unityGuid in unityGuids)
                {
                    ...
                    {
                        string depPath = FR2_Cache.GetAssetPath(unityGuid);
                ...
                foreach (string fr2Guid in fr2Guids)
                {
                    ...
                    {
                        string depPath = FR2_Cache.GetAssetPath(fr2Guid);
        ...
        private void LogValidationResults(ValidationResult result, System.TimeSpan duration, bool exportToFile)
        {
            ...
            if (result.assetsWithDifferences == 0)
            {
                ...
                FR2_LOG.Log("🎉 Perfect match! FR2 and Unity have identical reference detection.");
            }
            else
            {
                ...
                FR2_LOG.Log($"[FR2_VALIDATION] Accuracy: {accuracy:F1}%");
            ...
            if (exportToFile)
            {
                string filePath = System.IO.Path.Combine(Application.dataPath, "../FR2_Validation_Report.txt");
                System.IO.File.WriteAllText(filePath, reportBuilder.ToString());
                FR2_LOG.Log($"[FR2_VALIDATION] Detailed report exported to: {filePath}");
            }
            else
            {
                // Console output (limited)
                FR2_LOG.Log("=== FR2 REFERENCE VALIDATION RESULTS ===");
                FR2_LOG.Log($"Analysis completed in {duration.TotalSeconds:F2} seconds");
                FR2_LOG.Log($"Total assets analyzed: {result.totalAssets}");
                FR2_LOG.Log($"Assets with differences: {result.assetsWithDifferences}");
                if (result.assetsWithDifferences > 0)
                {
                    FR2_LOG.Log($"Accuracy: {((result.totalAssets - result.assetsWithDifferences) * 100f / result.totalAssets):F1}%");
                    FR2_LOG.Log("Use 'Validate References (Export to File)' for detailed analysis.");
                }
            }
        }
