...
{
    internal static class FR2_Define
    {
        internal static bool IsDebugModeEnabled()
        {
            string cscPath = GetCscFilePath();
            return System.IO.File.Exists(cscPath) && HasDefine(System.IO.File.ReadAllText(cscPath), "FR2_DEBUG");
        }
        ...
        internal static void ToggleDebugMode(bool enable)
        {
            ...
            content = enable ? AddDefine(content, "FR2_DEBUG") : RemoveDefine(content, "FR2_DEBUG");
            ...
            Debug.Log($"FR2 Developer Mode {(enable ? "enabled" : "disabled")}. Unity will recompile.");
        }
