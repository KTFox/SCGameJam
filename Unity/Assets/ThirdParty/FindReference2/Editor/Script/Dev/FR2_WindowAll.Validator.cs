...
namespace vietlabs.fr2
{
    internal partial class FR2_WindowAll
    {
        ...
        {
#if FR2_DEV
            menu.AddItem(new GUIContent("Refresh Cache"), false, () => FR2_Cache.Check4Changes(true));
        ...
        private void DebugSelectedAssets()
        {
            ...
            foreach (var obj in Selection.objects)
            {
                ...
                FR2_LOG.Log($"[FR2_DEBUG] === {obj.name} ({guid}) ===");
                ...
                FR2_LOG.Log($"Type: {asset.type} | Critical: {asset.IsCriticalAsset()} | Extension: {asset.extension}");
                FR2_LOG.Log($"Uses: {asset.UseGUIDs?.Count ?? 0} | UsedBy: {asset.UsedByMap?.Count ?? 0}");
                FR2_LOG.Log($"InAssetList: {FR2_Cache._inst.AssetList?.Contains(asset) ?? false} | Dirty: {asset.isDirty}");
            }
        }
#endif
    }
