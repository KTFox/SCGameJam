...
namespace vietlabs.fr2
{
    internal class FR2_TreeUI2
    {
        ...
        public void Reset(List<string> root)
        {
        ...
        public void Draw(Rect rect)
        {
            ...
            var contentRect = new Rect(0f, 0f, 1f, rootItem.childrenHeight + FR2_Theme.Current.TreeContentPadding * 2);
            ...
            position = GUI.BeginScrollView(visibleRect, position, contentRect);
            {
                ...
                var r = new Rect(0f, theme.TreeContentPadding, 
                    rect.width - (noScroll ? theme.CompactSpacing : theme.ScrollBarWidth) - itemPaddingRight, 
            ...
            if (Event.current.type == EventType.ScrollWheel && visibleRect.Contains(Event.current.mousePosition))
            {
                Event.current.Use();
            }
        }
        ...
        internal class Drawer
        {
            ...
            {
                return (int)FR2_Theme.Current.TreeItemHeight;
            }
            ...
            public virtual IReadOnlyList<string> GetChildren(string id)
            {
                return null;
            }
        ...
        internal class GroupDrawer : Drawer
        {
            ...
            {
                if (groupDict != null) groupDict.Remove(groupId);
                if (tree?.rootItem?.children == null) return;
                var children = tree.rootItem.children;
                for (int i = children.Count - 1; i >= 0; i--)
                {
                    if (children[i].id != groupId) continue;
                    tree.rootItem.childrenHeight -= children[i].height + children[i].childrenHeight + (int)FR2_Theme.Current.TreeItemSpacing;
                    children.RemoveAt(i);
                    tree.rootItem.childCount--;
                    break;
                }
            }
                ...
                Action<List<string>> customGroupSort = null)
            {
                ...
                if (hideGroupIfPossible && groupDict.Count == 1) //single group : Flat list
                {
                    List<string> v = null;
                    foreach (var kvp in groupDict) { v = kvp.Value; break; }
                    tree.Reset(v);
                    groupDict.Clear();
                }
                else //multiple groups
                {
                    var groups = new List<string>(groupDict.Count);
                    foreach (var kvp in groupDict) groups.Add(kvp.Key);
            ...
            {
                if (id == null || groupDict == null) return 0;
                return groupDict.TryGetValue(id, out List<string> group) ? group.Count : 0;
            }

            public override IReadOnlyList<string> GetChildren(string id)
            {
                if (groupDict == null) return null;
                return groupDict.TryGetValue(id, out List<string> group) ? group : null;
            }
        ...
        internal class TreeItem
        {
            ...
            internal void Draw(ref int index, ref Rect rect, int minY, int maxY)
            {
                ...
                if ((height > 0) && (interMin || interMax))
                {
                    ...
                    {
                        using (FR2_Scope.GUIColor(FR2_Setting.RowColor))
                        {
                    ...
                    var theme = FR2_Theme.Current;
                    float x = (depth + 1) * theme.TreeIndentSize + tree.itemPaddingLeft;
                    tree.drawer.Draw(new Rect(x, rect.y, rect.width - x, rect.height), this);

                    if (childCount > 0)
                    {
                        bool newIsOpen = GUI.Toggle(new Rect(x - theme.TreeIndentSize, rect.y, theme.TreeToggleSize, theme.TreeToggleSize), 
                        ...
                        if (newIsOpen != IsOpen)
                        {
                            ...
                            if (ctrl)
                            {
                                ApplySameActionToSiblings(newIsOpen);
                            }
                            else if (alt)
                            {
                                InvertSiblingsState();
                ...
                {
                    rect.y += height + FR2_Theme.Current.TreeItemSpacing;
                }
            ...
            internal void RefreshChildren(IReadOnlyList<string> childrenIDs)
            {
                ...
                for (var i = 0; i < childCount; i++)
                {
                    ...
                    childrenHeight += item.height + (int)FR2_Theme.Current.TreeItemSpacing;
                    children.Add(item);
                }
            }
            
            internal void ApplySameActionToSiblings(bool newState)
            {
                ...
                foreach (var sibling in parent.children)
                {
                    ...
                    {
                        sibling.IsOpen = newState;
                    }
                }
            }
            
            internal void InvertSiblingsState()
            {
                if (parent == null || parent.children == null) return;
