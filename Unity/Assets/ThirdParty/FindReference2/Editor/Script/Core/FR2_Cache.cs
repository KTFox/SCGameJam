...
namespace vietlabs.fr2
{
    internal partial class FR2_Cache : ScriptableObject
    {
        ...
        private void OnWindowFocusChanged(EditorWindow window)
        {
            ...
            // When FR2 window gains focus and has dirty assets, schedule a deferred refresh.
            // IncrementalRefresh only queues work via CheckDelayRefresh which waits for editor idle.
            if (isFR2Window && !wasFocused && FR2_SettingExt.isAutoRefreshEnabled && hasDirtyAsset)
            {
                // Don't refresh if editor is still importing — it will refresh when idle
