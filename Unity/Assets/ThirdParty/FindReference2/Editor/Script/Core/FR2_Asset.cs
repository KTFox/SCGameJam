...
namespace vietlabs.fr2
{
    ...
    internal partial class FR2_Asset
    {
        ...
        // Constants moved to FR2_Asset.Constants.cs
        ...
        [FormerlySerializedAs("type2")]
        [SerializeField] public AssetType type;
        ...
        [NonSerialized] internal int refreshStamp;
        ...
        // Path info moved to FR2_Asset.PathInfo.cs


        // Do not cache
        [NonSerialized] internal AssetState state;
        [NonSerialized] private Dictionary<string, FR2_Asset> _usedByMap;// = new Dictionary<string, FR2_Asset>();
        
        
        public Dictionary<string, FR2_Asset> UsedByMap
        {
            get
            {
                return _usedByMap ??= new Dictionary<string, FR2_Asset>();
            }
        }
        
        // Internal method for ClearUsedBy to clear without creating new dictionary
        internal void ClearUsedByInternal()
        {
            if (_usedByMap != null && _usedByMap.Count > 0 && type == AssetType.SCENE)
            {
                FR2_LOG.LogWarning($"[ClearUsedByInternal] Clearing scene {assetPath} with {_usedByMap.Count} references!");
            }
            _usedByMap?.Clear();
        }
        ...
        internal bool IsCriticalAsset()
        {
            ...
            return !NON_REFERENCE_EXTENSIONS.Contains(extension);
        }

        // GUID management methods moved to FR2_Asset.GuidManager.cs
        ...
        // AddUseGUID methods moved to FR2_Asset.GuidManager.cs
        ...
        internal static List<FR2_Asset> FindUsage(FR2_Asset asset)
        {
            ...
            List<FR2_Asset> refs = FR2_Cache._inst.FindAssets(asset.UseGUIDs.Keys.ToArray(), true);
        ...
        {
            return asset._usedByMap.Values.ToList();
        }
        ...
        internal bool ReplaceReference(string fromGUID, string toGUID, TerrainData terrain = null)
        {
            ...
            if (IsReferencable)
            {
                ...
                {
                    state = AssetState.MISSING;
                    FR2_LOG.LogWarning("Replace Reference error :: " + e + "\n" + m_assetPath);
                }
            ...
            if (type == AssetType.TERRAIN)
            {
                ...
                if (fromObj is Texture2D tex)
                {
                    ...
                    FR2_Terrain.ReplaceTerrainTextureDatas(terrain, tex, (Texture2D)toObj);
                }
            ...
            FR2_LOG.LogWarning("Something wrong, should never be here - Ignored <" + m_assetPath + "> : not a readable type, can not replace ! " + type);
            return false;
        }
        ...
        internal string ReplaceFileIdIfNeeded(string line, long toFileId)
        {
            ...
            if (endIndex > startIndex)
            {
                ...
                {
                    FR2_LOG.Log($"ReplaceReference: fromFileId {fromFileId} to File Id {toFileId}");
                    return line.Replace(fromFileId, toFileId.ToString());
                }
                FR2_LOG.LogWarning($"[Skip] Difference file type: {fromFileId} -> {toFileId}");
            } else
            {
                FR2_LOG.LogWarning("Cannot parse fileID in the line.");
            }
            return line;
        }
        ...
        internal bool ReplaceReference(string fromGUID, string toGUID, long toFileId, TerrainData terrain = null)
        {
            ...
            if (type == AssetType.TERRAIN)
            {
                ...
                if (fromObj is Texture2D)
                {
                    ...
                    terrain.detailPrototypes = arr;
                    FR2_Terrain.ReplaceTerrainTextureDatas(terrain, (Texture2D)fromObj, (Texture2D)toObj);
                }
            ...
            FR2_LOG.LogWarning("Something wrong, should never be here - Ignored <" + m_assetPath +
