    ...
    internal class FR2_SelectionManager
    {
        ...
        public static FR2_SelectionManager Instance
        {
            get
            {
                ...
                {
                    FR2_LOG.LogWarning("Should not initialize SelectionManager before FR2_Cache ready!");
                    return null;
                }
        ...
        private static void Initialize()
        {
            ...
            AssemblyReloadEvents.beforeAssemblyReload += Cleanup;
            AssemblyReloadEvents.beforeAssemblyReload -= ClearInstanceIdCaches;
            AssemblyReloadEvents.beforeAssemblyReload += ClearInstanceIdCaches;
        }
        ...
        private void ScheduleNotification()
        {
            ...
            EditorApplication.delayCall += NotifySelectionChanged;
        }
        ...
        private void NotifySelectionChanged()
        {
            ...
            if (FR2_Cache.status != FR2_Status.Ready) return;
            
            FR2_Event.DispatchGlobal<SelectionChangedEvent>();
        }
        ...
        {
            if (obj == null) return;
            var instanceId = FR2_Unity.GetInstanceId(obj);
        ...
        {
            if (FR2_Cache.TryGetGUIDAndLocalFileIdentifier(obj, out _, out _))
            {
        ...
        {
            FR2_Cache.ClearPerformanceCaches();
            sceneObjectInstanceIds.Clear();
        }
        
        public static (int guidCacheSize, int sceneObjectCacheSize) GetCacheStats()
        {
            return (FR2_Cache.instanceIdToGuidLocalIdCache.Count, sceneObjectInstanceIds.Count);
        }
        
        public static (string guid, long localId) GetCachedGuidAndLocalId(UnityObject obj)
        {
            FR2_Cache.TryGetGUIDAndLocalFileIdentifier(obj, out string guid, out long localId);
            return (guid, localId);
        }
        ...
        internal class FR2_SceneSelection
        {
            ...
            {
                return go != null && instanceIds.Contains(FR2_Unity.GetInstanceId(go));
            }
            ...
            public GameObject[] ToArray()
            {
                if (arrayDirty)
                {
                    // OPTIMIZED: Replace LINQ with manual filtering to avoid allocations
                    var validGameObjects = new List<GameObject>(gameObjects.Count);
                    ...
                    cachedGameObjectArray = validGameObjects.ToArray();
                    arrayDirty = false;
                }
        ...
        internal class FR2_AssetSelection
        {
            ...
            {
                if (obj == null) return;
                AddAssetCached(obj, FR2_Unity.GetInstanceId(obj));
            }
            ...
            public void AddAssetCached(UnityObject obj, int instanceId)
            {
                ...
                // Check if we already have GUID and localId cached
                if (FR2_Cache.TryGetGUIDAndLocalFileIdentifier(obj, out string guid, out long fileId))
                {
                    if (!assets.ContainsKey(guid))
                    {
                        string cachedAssetPath = GetAssetPath(guid);
                        AddAssetEntry(guid, fileId, cachedAssetPath);
                    }
                    return;
                }

                if (FR2_Cache.TryGetGUIDAndLocalFileIdentifier(obj, out string objGuid, out long objFileId))
                {
                    string assetPath = GetAssetPath(objGuid);
                    AddAssetEntry(objGuid, objFileId, assetPath);
                }
            }
            
            private (string guid, long fileId) GetGuidAndFileId(UnityObject obj)
            {
                return GetCachedGuidAndLocalId(obj);
            }
            
            
            private string GetAssetPath(string guid)
            {
                return FR2_Cache.GUIDToAssetPath(guid);
            }
            ...
            {
                if (FR2_Cache.TryGetGUIDAndLocalFileIdentifier(obj, out _, out long fileId))
                {
                    return fileId;
                }
                return -1;
            ...
            public string[] GetGuids()
            {
                if (guidArrayDirty)
                {
                    // OPTIMIZED: Avoid LINQ ToArray() allocation
                    cachedGuidsArray = new string[assets.Count];
                    int index = 0;
                    foreach (string guid in assets.Keys)
                    {
                        cachedGuidsArray[index++] = guid;
                    }
