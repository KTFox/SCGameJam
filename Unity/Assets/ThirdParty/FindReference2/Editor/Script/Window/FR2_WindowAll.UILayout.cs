...
namespace vietlabs.fr2
{
    internal partial class FR2_WindowAll
    {
        ...
        private void RefreshPanelVisible()
        {
            ...
            sp2.SetSplitVisible(0, isScenePanelVisible);
            sp2.SetSplitVisible(1, isAssetPanelVisible);
            sp2.SetSplitVisible(2, isFocusingAddressable);
        }

        private void MarkDirty()
        {
            if (_allDrawersCache != null)
                foreach (var drawer in _allDrawersCache) drawer?.SetDirty();
        ...
        {
            if (_allDrawersCache != null)
                foreach (var drawer in _allDrawersCache) drawer?.RefreshSort();

            AddressableDrawer.RefreshSort();
            Duplicated.RefreshSort();
            UsedInBuild.RefreshSort();
            bookmark?.RefreshSort();
            if (settings.toolMode) RefUnUse?.RefreshSort();
            Repaint();
        }
        ...
        private FR2_RefDrawer GetSceneDrawer()
        {
            if (isFocusingUses) return IsSelectingAssets ? null : SceneUsesDrawer;
            if (isFocusingUsedBy) return IsSelectingAssets ? RefInScene : RefSceneInScene;
            return null;
        }
    }
}
