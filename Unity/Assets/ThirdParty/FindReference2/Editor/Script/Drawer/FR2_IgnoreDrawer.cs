...
namespace vietlabs.fr2
{
    internal class FR2_IgnoreDrawer
    {
        ...
        private readonly RowDrawData _ignoreRowData = new RowDrawData();
        private bool dirty;
        ...
        private void DrawItem(Rect r, string guid)
        {
            ...
            {
                using (FR2_Scope.GUIColor(Color.blue))
                {
                    GUI.DrawTexture(new Rect(r.x - 4f, r.y + 2f, 2f, 2f), EditorGUIUtility.whiteTexture);
                }
            }

            rf.asset.PopulateRowData(_ignoreRowData, true, null, null, rf);
            _ignoreRowData.ClearColumns();
            FR2_RowDrawer.Draw(r, _ignoreRowData);
        ...
        public void Draw()
        {
            ...
            {
                GUILayout.Space(4f);
                UnityObject[] drops = GUI2.DropZone("Drag & Drop folders here to exclude", 100, 95);
        ...
        private void ApplyFiter()
        {
            ...
            {
                string guid = FR2_Cache.AssetPathToGUID(item2);
                if (string.IsNullOrEmpty(guid)) continue;

                FR2_Asset asset = FR2_Cache.GetAsset(guid, true);
