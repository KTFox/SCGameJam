...
{
    internal class FR2_MissingReference : IRefDraw
    {
        ...
        internal FR2_MissingReference(IWindow window, Func<FR2_RefDrawer.Sort> getSort, Func<string> getGroup)
        {
            _window = window;
        ...
        public bool DrawLayout()
        {
            using (VtLayout())
            {
                ...
                {
                    if (GUILayout.Button("Scan & Remove Missing Scripts", FR2_Theme.Current.ActionButtonHeight))
                    {
                        StartProcessing();
                    }
        ...
        private void StartProcessing()
        {
            ...
            {
                FR2_LOG.LogError($"Error starting processing: {ex.Message}");
        ...
        private void ProcessNextAsset()
        {
            ...
            {
                FR2_LOG.LogError($"Error processing {assetPath}: {ex.Message}");
        ...
        private void SaveCurrentSceneSetup()
        {
            ...
            for (int i = 0; i < FR2_Unity.LoadedSceneCount; i++)
            {
        ...
        private void RestoreOriginalScenes()
        {
            ...
            {
                FR2_LOG.LogError($"Error restoring original scenes: {ex.Message}");
            }
        }
        ...
        private void AddAssetTypeToPipeline(string assetTypeFilter)
        {
            ...
            {
                _assetsToProcess.Add(FR2_Cache.GUIDToAssetPath(guid));
            }
        }
        ...
        private void ProcessPrefab(string prefabPath)
        {
            ...
            {
                FR2_LOG.LogError($"Error processing prefab {prefabPath}: {ex.Message}");
        ...
        {
            if (_currentlyOpenScene.HasValue && FR2_Unity.GetActiveScene().isDirty)
            {
                EditorSceneManager.SaveScene(FR2_Unity.GetActiveScene());
                _currentlyOpenScene = null;
            }
