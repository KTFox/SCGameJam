...
namespace vietlabs.fr2
{
    public static partial class FR2_Addressable
    {
        ...
        {
            string folderPath = FR2_Cache.GUIDToAssetPath(guid);
            if (!AssetDatabase.IsValidFolder(folderPath)) return;
            string[] allGUIDs = AssetDatabase.FindAssets("*", new[] { folderPath });
            foreach (string child in allGUIDs)
            {
                FR2_Asset asset = FR2_Cache.GetAsset(child, true);
                if (asset == null)
                {
                    FR2_LOG.LogWarning($"Why asset is null? {guid}\n{folderPath}");
                    continue;
                }
                
                if (asset.IsExcluded || asset.IsMissing || asset.IsScript || asset.type == FR2_Asset.AssetType.UNKNOWN) continue;
