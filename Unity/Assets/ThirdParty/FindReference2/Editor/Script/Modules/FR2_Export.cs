...
namespace vietlabs.fr2
{
    internal class FR2_Export
    {
        ...
        public static void ExportCSV(FR2_Ref[] csvSource)
        {
            ...
            {
                FR2_LOG.LogWarning("[FR2] Nothing to export!");
            }
        }
        ...
        private static void DebugDetails()
        {
            ...
            {
                FR2_LOG.LogWarning("Path: " + path);
                return;
            }
            
            var guid = FR2_Cache.AssetPathToGUID(path);
            var asset = FR2_Cache.GetAsset(guid);
            if (asset == null)
            {
                FR2_LOG.LogWarning($"Asset is null??? {guid}");
                return;
            }
        ...
        private static void Ignore()
        {
            ...
            {
                FR2_LOG.LogWarning("FR2 cache not yet ready, please open Window > FR2_Window and hit scan project!");
                return;
            }
        ...
        {
            EditorGUIUtility.systemCopyBuffer = FR2_Cache.AssetPathToGUID(
        ...
        public static void ForceRefreshSelection()
        {
            ...
            {
                string guid = guids[i];
                if (FR2_Cache._instGUID == guid) continue;
        ...
        public static void MergeDuplicate(string guid_file)
        {
            ...
            string gPath = FR2_Cache.GUIDToAssetPath(guid);
            if (string.IsNullOrEmpty(gPath) || !gPath.StartsWith("Assets/"))
            {
                FR2_LOG.LogWarning("Invalid guid <" + guid + "> in clipboard, can not replace !");
                return;
            }
            ...
            {
                FR2_LOG.LogWarning("No new asset selected to replace, must select all duplications to replace");
                return;
            }
...
#if REPLACE_SAME_TYPE
                var p2 = FR2_Cache.GUIDToAssetPath(g);
                var type2 = AssetDatabase.GetMainAssetTypeAtPath(p2);

                if(type1 != type2)
                {
                    FR2_LOG.LogWarning("Cannot replace asset: " + p2 + " becase difference type");
                    continue;
                }
            ...
            assetList = FR2_Cache._inst.FindAssets(lstFind.ToArray(), false);
...
//                Debug.Log("FR2 Replace GUID : " + assetList[i].guid + " ---> " + guid + " : " + assetList[i].UsedByMap.Count + " assets updated");
        ...
        private static void ApplicationUpdate()
        {
            ...
            foreach (KeyValuePair<string, ProcessReplaceData> item in listReplace)
            {
                ...
                for (var i = 0; i < item.Value.datas.Count; i++)
                {
                    ...
                    a.isTerrian = a.asset.type == FR2_Asset.AssetType.TERRAIN;
                    if (a.isTerrian)
                    {
                ...
                for (var i = 0; i < lst.Count; i++)
                {
                    ...
                    if (!data.isUpdated && data.isSucess)
                    {
                        ...
                        } else
                        {
                            ...
                            {
                                FR2_LOG.LogWarning(data.asset.assetPath + "\n" + e);
                            }
                        }
                    }
                }
                ...
                foreach (KeyValuePair<string, ProcessReplaceData> item in listReplace)
                {
                    ...
                    for (var i = 0; i < lst.Count; i++)
                    {
                        ...
                        if (data.isSucess && !guidsRefreshed.Contains(data.asset.guid))
                        {
                            ...
                            FR2_Cache.RefreshAsset(data.asset.guid, true);
                        }
                    }
                }
            }
        ...
        {
            List<FR2_Asset> list = FR2_Cache._inst.FindAssets(FR2_Unity.Selection_AssetGUIDs, false);
        ...
        //	var oGUID = FR2_Cache.AssetPathToGUID(oAssetPath);
