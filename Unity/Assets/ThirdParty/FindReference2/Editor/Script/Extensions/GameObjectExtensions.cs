...
namespace vietlabs.fr2
{
    internal static class GameObjectExtensions
    {
        ...
        {
            for (var j = 0; j < FR2_Unity.SceneCount; j++)
            {
                var scene = FR2_Unity.GetSceneAt(j);
        ...
        internal static bool HasAnyLoadedScene()
        {
            for (int i = 0; i < FR2_Unity.SceneCount; i++)
            {
                if (FR2_Unity.GetSceneAt(i).isLoaded) return true;
            }
            return false;
        }
